...
(function () {
  ...
  // 通過設置HTML元素的類，我們可以更早地應用語言樣式
  document.documentElement.className = savedLanguage;
})();
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (languageSelect) {
    ...
    const savedLanguage =
      localStorage.getItem("preferred-language") || "zh-hant";
    // 不需要再設置 document.documentElement.lang，已經在頁面加載時設置了
    ...
    languageSelect.addEventListener("change", function (e) {
      ...
      document.documentElement.className = selectedLanguage; // 更新html的类名
  ...
  if (hero) {
    ...
    if (gradientSphere) {
      window.addEventListener("mousemove", function (e) {
        ...
        gradientSphere.style.transform = `translate(${x * 20 - 10}px, ${
          y * 20 - 10
        }px)`;
      });
    }
